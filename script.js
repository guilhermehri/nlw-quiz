...
const perguntas = [ //array
  ...
  const corretas = new Set() //conjunto, não repete informações
  const totalDePerguntas = perguntas.length
  const mostrarTotal = document.querySelector('#acertos span') //pega span do id acertos
  mostrarTotal.textContent = corretas.size + ' de ' + totalDePerguntas
  
  for(const item of perguntas) {
    const quizItem = template.content.cloneNode(true) /* clona o nó de template e atribui ao quizItem */
    quizItem.querySelector('h3').textContent = item.pergunta
    //procura a tag h3 de cada item de perguntas, e altera o conteúdo

    for(let resposta of item.respostas) {
      const dt = quizItem.querySelector('dl dt').cloneNode(true)
      //procura dt em dl
      dt.querySelector('span').textContent = resposta
      dt.querySelector('input').setAttribute('name', 'pergunta-'+perguntas.indexOf(item)) //adiciona atributos //pesquisa índices em item
      dt.querySelector('input').value = item.respostas.indexOf(resposta)
      dt.querySelector('input').onchange = (event) => { //arrow function
      ...
      //onchange: quando o valor é alterado
  
      quizItem.querySelector('dl').appendChild(dt) //adiciona conteúdo
